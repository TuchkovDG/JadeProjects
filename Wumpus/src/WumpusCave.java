...
public class WumpusCave {
	...
	private Position start = new Position(1, 1, Position.Orientation.FACING_NORTH);
	private Room wumpus;
	private Room gold;
	...
	public WumpusCave(int caveXDimension, int caveYDimension, String config) {
		...
		for (int i = 0; i < config.length(); i++) {
			...
			switch (c) {
				case 'S': start = new Position(r.getX(), r.getY(), Position.Orientation.FACING_NORTH); break;
	...
	public Position moveForward(Position position) {
		...
		return allowedRooms.contains(room) ? new Position(x, y, position.getOrientation()) : position;
	}

	public Position turnLeft(Position position) {
		Position.Orientation orientation = null;
		switch (position.getOrientation()) {
			case FACING_NORTH: orientation = Position.Orientation.FACING_WEST; break;
			case FACING_SOUTH: orientation = Position.Orientation.FACING_EAST; break;
			case FACING_EAST: orientation = Position.Orientation.FACING_NORTH; break;
			case FACING_WEST: orientation = Position.Orientation.FACING_SOUTH; break;
		}
		return new Position(position.getX(), position.getY(), orientation);
	}

	public Position turnRight(Position position) {
		Position.Orientation orientation = null;
		switch (position.getOrientation()) {
			case FACING_NORTH: orientation = Position.Orientation.FACING_EAST; break;
			case FACING_SOUTH: orientation = Position.Orientation.FACING_WEST; break;
			case FACING_EAST: orientation = Position.Orientation.FACING_SOUTH; break;
			case FACING_WEST: orientation = Position.Orientation.FACING_NORTH; break;
		}
		return new Position(position.getX(), position.getY(), orientation);
	}
